...
#   uninstall <技能...>           卸载（移除符号链接；拷贝模式需确认）
...
while [ $# -gt 0 ]; do
  case "$1" in
    --target) TARGET="$2"; PASSTHRU+=(--target "$2"); shift 2 ;;
    --copy) MODE="copy"; PASSTHRU+=(--copy); shift ;;
    --with-deps) WITH_DEPS=1; PASSTHRU+=(--with-deps); shift ;;
    --yes|-y) ASSUME_YES=1; PASSTHRU+=(--yes); shift ;;
    *) ITEMS+=("$1"); PASSTHRU+=("$1"); shift ;;
  esac
done
...
if [ -n "${SCRIPT_DIR}" ] && [ -f "${SCRIPT_DIR}/../skills.json" ]; then
  ...
  exec "${REPO_DIR}/scripts/dsh-skill.sh" "${CMD}" ${PASSTHRU[@]+"${PASSTHRU[@]}"}
fi
...
for name,meta in d.items():
    print('  %-20s [%s] %s' % (name, meta['type'], meta['summary']))
"
    ;;
  install)
    ...
    for it in ${ITEMS[@]+"${ITEMS[@]}"}; do install_one "${it}"; done
  ...
  doctor)
    ...
    while IFS= read -r name; do
      if [ -e "${TARGET}/${name}" ]; then
        if [ -f "${TARGET}/${name}/SKILL.md" ]; then echo "  ✓ ${name}：SKILL.md 在位"; else echo "  ✗ ${name}：缺 SKILL.md"; fi
      else
        echo "  · ${name}：未安装"
      fi
    done < <(skill_names)
