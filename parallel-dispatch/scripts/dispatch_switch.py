...
FANOUT_TOOL_NAMES = slog.DSH_FANOUT_TOOLS + slog.KIMI_FANOUT_TOOLS


def resolve_session_log(session_log=None):
    """定位当前会话日志（DSH/Kimi 双格式），返回 (path, source)。"""
...
def scan_turn_todos(path):
    ...
    turn 起点之后的最后一次清单登记（DSH todo/write / Kimi TodoList）。
    """
...
def main():
    ...
    ap.add_argument("--session-log", type=str, default=None,
                    help="掷点 A 前置校验用：显式指定当前会话日志路径（DSH session.jsonl.zstd 或 Kimi wire.jsonl；缺省自动定位活跃日志）")
