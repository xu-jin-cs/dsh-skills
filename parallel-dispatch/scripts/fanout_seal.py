...
def check_baseline_reads(children, baselines, sessions_root, session_log_path=None):
    ...
    DSH 模式：子分身日志在 sessions_root/*/child/session.jsonl.zstd；
...
def seal(session_log=None, switch_log=DEFAULT_SWITCH_LOG, sessions_root=None, project_root=None):
    ...
    if path is None:
        return {
            "verdict": "CLARIFY",
            "reason": "无法定位当前会话日志（DSH/Kimi 均未命中）→ 用 --session-log 显式指定后重扳",
        }, EXIT_CLARIFY
...
def main():
    ...
    ap.add_argument("--session-log", type=str, default=None,
                    help="显式指定当前会话日志路径（DSH session.jsonl.zstd 或 Kimi wire.jsonl；缺省 DSH_SESSION_JSONL/活跃启发式）")
