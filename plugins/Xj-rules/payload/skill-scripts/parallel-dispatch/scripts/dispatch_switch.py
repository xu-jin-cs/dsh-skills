...
FANOUT_TOOL_NAMES = ("subagent", "subagent_fork", "workflow")


def resolve_session_log(session_log=None):
    """定位当前会话日志，返回 (path, source)；无法定位返回 (None, None)。

    权威顺序：① --session-log 显式指定；② DSH_SHELL 注入的 DSH_SESSION_JSONL
    环境变量（精确指向扳动者所在会话，多窗口并发也不串线）；③ 活跃窗口内
    mtime 最新的日志（非 DSH 托管 shell 的降级启发式，可能受并行窗口干扰）。
    """
    if session_log:
    ...
    for path in glob.glob(DSH_SESSIONS_GLOB):
        try:
            ...
            continue
        if now - mtime > TODO_GATE_FRESH_SECONDS:
            continue
...
def scan_turn_todos(path):
    """扫描会话日志，返回当前 turn 的 todo/write 条目数；无登记返回 0，解压失败返回 None。

    todos 投影随 turn/start 清空（last-write-wins），故只认最后一个
    turn/start 之后的最后一次 todo/write。
    """
    try:
    ...
    for raw in proc.stdout.decode("utf-8", errors="replace").splitlines():
        ...
        if '"type":"turn/start"' in raw:
            ...
            cur_todos = len(todos) if isinstance(todos, list) else 0
...
def todo_gate_check(n, session_log=None):
    ...
    if path is None:
        return {"ok": None,
                "reason": "无法定位当前 DSH 会话日志（无 DSH_SESSION_JSONL 且活跃窗口内无 session.jsonl.zstd）→ 用 --session-log 显式指定后重扳"}
    ...
    if count == 0:
        return {"ok": False, "log": path, "log_source": source, "todos": 0,
                "reason": "当前 turn 无 todo/write 事件 → 扇出前任务清单未登记（GUI 无清单/进度可展示）"}
...
def probe_gate_check(units, session_log=None):
    ...
    try:
        ...
        return {"ok": None, "reason": "会话日志解压失败：%s（需 zstd CLI 在 PATH 中）" % path}
    if proc.returncode != 0:
        return {"ok": None, "reason": "会话日志解压失败：%s（需 zstd CLI 在 PATH 中）" % path}
    ...
    settled = set()
    for raw in proc.stdout.decode("utf-8", errors="replace").splitlines():
        if '"type":"tool/call"' in raw and any('"name":"%s"' % t in raw for t in FANOUT_TOOL_NAMES):
            try:
                obj = json.loads(raw)
            except json.JSONDecodeError:
                continue
            data = obj.get("data", {})
            calls[data.get("callId")] = obj.get("time", 0)
        elif '"type":"tool/result"' in raw:
            i = raw.find('"callId":"')
            if i != -1:
                j = raw.find('"', i + 10)
                if j != -1:
                    settled.add(raw[i + 10:j])
...
def main():
    ...
    ap.add_argument("--session-log", type=str, default=None,
                    help="掷点 A 前置校验用：显式指定当前会话 session.jsonl.zstd 路径（缺省自动定位活跃日志）")
