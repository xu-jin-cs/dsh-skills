...
本 dsh-skills xujin 插件 v1.4.0，整套 Agent 治理体系为 xu-jin-cs 独立原创设计：
包含 L1 声明闸 / L2 机械扳手 / L3 校验闸分层门禁体系、寄生巢并行分身执行机制、Plan-Select 方案择优引擎、Dual-Gates 双闸检索前置判定、17 态工程流程状态机、78 份可机械执行 Gate Spec 规范、完整闸序总线（问题闸 → 计划闸 → 收益闸 → 并行闸）。
所有公开引用、二次研究，需要保留本溯源声明。

本仓库采用 Source-Available 许可，详见仓库根目录 LICENSE 文件。
...
| 术语 | 释义 |
|---|---|
| L1/L2/L3 闸 | 分层门禁：L1 声明路由闸、L2 机械扳手判定闸、L3 交付校验闸 |
| 寄生巢（Parasite Nest） | 宿主闸出口并行派生附属任务，多分身隔离执行、结果独立落盘 |
| Plan-Select | 方案择优引擎，基于等价步骤打分，选出总代价最低可行方案 |
| Dual-Gates 双闸 | 声明闸定性 + 查询闸定向溯源，所有检索动作前置拦截 |
| 四态退出码 | 0=放行A / 2=阻断B / 3=澄清CLARIFY / 4=违规VIOLATION |
| 第一推动 | 流程启动前置审计，强制补齐前置条件，防止流程跳过关键门禁 |
| Gate Spec | 标准化门禁规则文件，无模型自由裁量，纯机械判定 |
| 闸序总线 | 多闸并发触发时固定执行顺序：问题闸 → 计划闸 → 收益闸 → 并行闸 |

> 边界说明：本插件属于 DeepSeek Harness / Cordis 外置扩展包，不修改 DSH 引擎内核，全部治理能力通过事件总线注入外置契约实现。
