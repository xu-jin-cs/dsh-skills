...
本闸克隆 reform_exit_guard.py 骨架，扫 DSH 会话 jsonl + 各开关留痕 jsonl，
检测"模式信号在、对应开关留痕缺"的不对称（事后必被查 → 倒逼事前自觉扳）：
  ...
  F5 WARNING：流程特征（harness-step-sync / HARNESS_API_URL）出现但窗口内无
     engine_health 掷点 → 仅告警（流程启动特征枚举先窄后宽，防误报）
...
RETRO_CONFIG = os.path.expanduser("~/.agents/retro-skills-registry/dispatcher_generate_config.json")
RETRO_MATCH_OUTPUT = os.path.expanduser("~/.agents/retro-skills-registry/runtime/_active_match.md")
...
ENGINE_FLOW_SIGNALS = ["harness-step-sync", "HARNESS_API_URL", "orchestrator_gate"]
...
def main():
    ...
    # F5 WARNING：流程特征无 engine_health
    if any(sig in cmd for cmd in bash_cmds for sig in ENGINE_FLOW_SIGNALS):
        if not log_has_record(GATE_LOG, start_iso, lambda r: "engine_health" in str(r.get("spec", ""))):
            warnings.append({"code": "F5-ENGINE-NO-HEALTH",
                "detail": "会话出现 engine 流程特征命令但窗口内无 engine_health 掷点（WARNING，人工复核）"})
    ...
    try:
        ...
        _dg_path = os.path.expanduser("~/.dsh/xujin-scripts/skills/dual-gates/scripts/dual_gates.py")
