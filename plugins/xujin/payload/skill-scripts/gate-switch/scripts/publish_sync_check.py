...
两个用户根（~/.dsh/skills/、~/.dsh/xujin-scripts/skills/）必须是指向仓库的符号链接，
禁止双副本漂移；更新规则后须 git push。

一门三查（对仓库内每个技能）：
  P1 符号链接：~/.dsh/skills/<name> 与 ~/.dsh/xujin-scripts/skills/<name> 均为符号链接
...
ROOTS = [os.path.expanduser("~/.dsh/skills"), os.path.expanduser("~/.dsh/xujin-scripts/skills")]
