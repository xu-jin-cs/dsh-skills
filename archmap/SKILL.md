...
/archmap <项目路径> whitebox-pack          # 模式 H：生成 whitebox_data_pack.json（白盒数据支撑包）
```
...
### 模式 H：whitebox 数据支撑包（whitebox_data_pack，2026-09-21 REFORM-GATE 判A 新增）

为 `whitebox-coverage` 白盒静态分析提供**模块级影响图谱数据支撑**（代码依赖 / 数据链路 / 路由供需三维聚合）。**纯文本规程模式**——本模式不由 archmap 包装脚本路由、零新增脚本、零新扫描零 LLM，由 Agent 按本规程对既有产物机械聚合产出；属**派生数据**，真源（`full_index.json` / `file_imports.json` / `file_routes.json` / `module_hashes.json`）在则恒可重建（DIM-022）。

触发时机：

1. 消费方（whitebox-coverage 静态分析）要求提供数据支撑包时
2. full / sync / lite 执行后可按需补生成（基线产物已就绪即可聚合）

触发：

```
/archmap <项目路径> whitebox-pack
```

生成规程（逐字段来源映射，字段契约唯一权威 = 技能内 `contracts/whitebox_data_pack.schema.json`，相对技能根；聚合逻辑以契约 `fields.*.source` 描述为准）：

1. 读取 `<项目路径>/archmap/` 下四件既有产物：`full_index.json`、`file_routes.json`、`file_imports.json`、`module_hashes.json`，记录各自 mtime 入 `source_artifacts` 留痕
2. `baseline_fingerprint`：`module_hashes.json` 全量键值对 sha1 前 12 位 + `full_index.json` mtime（版本锚点 DIM-049，供消费方机械判陈旧）
3. 聚合 `modules[]`（模块影响图谱主体，粒度上限=api/方法级不变）：
   - `module_id` / `module_path` / `apis` / `storages` / `dependencies` / `dependents`：full_index `modules[]` 逐模块提取（apis/storages 原样）
...
9. 固定字段：`pack_version` = "1.0.0"、`project` = 项目根绝对路径、`generated_at` = ISO8601 生成时间；`truncated` / `warnings` 按下述护栏填记

产物：`<项目路径>/archmap/whitebox_data_pack.json`。

护栏（必须执行）：

- **原子写（DIM-032）**：先写临时文件再 `os.replace` 原子落盘，禁止半截产物
- **截断闸（DIM-050）**：`modules` 超过 200 时截断，置 `truncated=true` 并追加 WARN 级 `warnings`
- **陈旧判（DIM-049）**：`baseline_fingerprint` 与消费方重算的当前 `module_hashes.json` 指纹不一致即陈旧，消费方机械可判并要求重生成；archmap 不主动追踪消费方判定

与既有模式关系：本模式为**纯增量**能力，不改动 full / lite / sync / diff 任何既有行为与产物结构，仅消费既有产物做机械聚合。

### 模式 I：API 接口文档集合（api-doc，2026-09-22 用户裁定新增）

为下游消费方（api-test-engineer 平台录入模式等）提供**全量接口文档聚合产物**（人类可读 Markdown 文档 + 机器可读幂等键索引双产物）。**纯文本聚合规程模式**（同模式 H 先例）——本模式不由 archmap 包装脚本路由、零新增脚本、零新扫描、零 LLM，由 Agent 按本规程对既有产物机械聚合产出；属**派生数据**，真源（`full_index.json` / `file_routes.json` / `module_hashes.json`）在则恒可重建。

触发时机：full / sync / lite 基线就绪后按需执行（基线产物已就绪即可聚合）。
...
2. 文档标题 = **产品名**，取项目根目录名
3. **一级分类 = 功能**：按路由一级前缀推导（如 `/api/auth/*` → `auth` 认证功能域）；无法推导一级前缀的接口归「未分组」并 WARN 留痕
4. **二级分类 = 业务模块**：archmap 模块（`module_id` / `module_path`）；逐接口标注对应代码模块名称（`module_id` + 所属文件路径，复用 `06_API资产清单` 的路由→文件映射能力）
5. **逐接口条目**结构对齐 rest-api-doc-standard.txt 八章节骨架（基础信息 / 请求头 / 请求入参 / 返回出参 / 全局返回码 / 请求示例 / 返回成功示例 / 返回失败示例），章节顺序固定、表格列不可删减：静态可推导的字段填实值（接口URL、请求方式、路径参数等），静态不可推导的字段一律标「源码未声明」，**禁止编造（零虚构原则）**
6. 统计全文档「源码未声明」字段占比，占比 >50% 时在文档头部置 WARN 提示「本批次文档为骨架契约」

产物（双产物）：

- `<项目路径>/archmap/<产品名>_接口文档.md`：人类可读接口文档集合（产品名做文件名安全化：非法字符 `/\:*?"<>|` 与空格统一替换为 `-`）
- `<项目路径>/archmap/api_doc_index.json`：机器可读幂等键索引，每条 `{method, url, 功能分类, 业务模块, 代码模块module_id, source_file}`，供 api-test-engineer 增量比对消费

护栏（必须执行）：

- **原子写**：先写临时文件再 `os.replace` 原子落盘，禁止半截产物
- **截断闸**：单功能分类接口数 >100 时截断，并追加 WARN 留痕
