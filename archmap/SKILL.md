...
8. **有变更时自动留痕**：追加 `diff_history.jsonl`（机器可读，含备注）并重渲染 `10_变更历史.md`（时间+修改内容+变更文件区间+闭包+测试选择）；零变更运行不记录
...
1. **不归因不收尾**：存在 accepted 建议而 `internalizations.jsonl` 无过闸记录，任务不得标记完成
...
<!-- 由 dispatcher_generate.py 全量维护，勿手改；最近注入: 2026-08-16T00:08:02.534965 -->
