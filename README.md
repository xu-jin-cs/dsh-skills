...
通用、自包含的 **PM 全流程研发调度骨架**（13 节点：pm_bootstrap→spm→pm_prd_confirm→dpm→[ui_designer ∥ test_lead_design]→fe→be→pm_quality_gate→test_lead_full→ops→qa→process_audit→retro），默认引擎接线为同仓库 [`Xj-engine`](./Xj-engine/)（`engine.kernel.et` / `xj-engine` CLI），引擎可插拔。各节点角色由 `flow.yml` 的 `invoked_skills` 指向对应技能（适配方按自身生态替换）。含：
