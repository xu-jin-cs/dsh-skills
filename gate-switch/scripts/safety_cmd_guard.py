    ...
    for cmd in cmds:
        for line in re.split(r"&&|\|\||;", cmd):
            ...
            if RE_RM.search(seg):
                ...
                dst = tokens[-1] if len(tokens) >= 3 else None
                for src in sources:
                    if src.endswith("/") or "*" in src or os.path.isdir(os.path.expanduser(src)):
                        # 归档白名单（2026-09-06 用户审核通过"我允许你删，通过用户审核"）：
                        # 仅放行 源=~/.agents/skills/* → 目的地=~/.agents/archive/* 的整目录归档 mv，
                        # 其余目录 mv 一律仍判 S1-MV-DIRECTORY
                        src_abs = os.path.realpath(os.path.expanduser(src))
                        dst_abs = os.path.realpath(os.path.expanduser(dst)) if dst else ""
                        skills_root = os.path.realpath(os.path.expanduser("~/.agents/skills/"))
                        archive_root = os.path.realpath(os.path.expanduser("~/.agents/archive/"))
                        if (src_abs.startswith(skills_root + os.sep)
                                and dst_abs.startswith(archive_root + os.sep)):
                            continue
