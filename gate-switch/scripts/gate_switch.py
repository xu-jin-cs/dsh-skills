...
def _p(v):
    """路径字段统一支持 ~ 展开（向后兼容：绝对路径不受影响，公开分发的规格不含机器名）。"""
...
def run_check(c):
    ...
    label = c.get("label", t)
    if t == "file_exists":
        ok = os.path.exists(_p(c["path"]))
        return ok, f"{label}: {c['path']} {'存在' if ok else '不存在'}"
    if t == "file_min_size":
        if not os.path.isfile(_p(c["path"])):
            return False, f"{label}: {c['path']} 不存在"
        size = os.path.getsize(_p(c["path"]))
        return size >= c["bytes"], f"{label}: {size}B（下限 {c['bytes']}B）"
    if t == "json_field":
        if not os.path.isfile(_p(c["path"])):
            return False, f"{label}: {c['path']} 不存在"
        try:
            data = json.load(open(_p(c["path"]), encoding="utf-8"))
    ...
    if t == "glob_count":
        n = len(glob.glob(_p(c["pattern"]), recursive=True))
        return _cmp(c["op"], n, c["value"]), f"{label}: 计数={n} op={c['op']} expect={c['value']}"
    if t == "grep_count":
        n = 0
        for p in glob.glob(_p(c["path"]), recursive=True):
            if os.path.isfile(p):
    ...
    if t == "mtime_after":
        if not os.path.exists(_p(c["path"])):
            return False, f"{label}: {c['path']} 不存在"
        if not os.path.exists(_p(c["ref_path"])):
            return False, f"{label}: 参照 {c['ref_path']} 不存在"
        ok = os.path.getmtime(_p(c["path"])) > os.path.getmtime(_p(c["ref_path"]))
        return ok, f"{label}: 产物{'新于' if ok else '不新于'}参照"
    if t == "script_exit":
        r = subprocess.run(os.path.expandvars(c["cmd"]), shell=True, capture_output=True, text=True, timeout=300)
